...
import (
	...
	"ent_cassandra_sandbox/ent" // Import the generated ent models
)
...
func createCart(session *gocql.Session, cart *ent.ShoppingCart) error {
...
func getCartByUserID(session *gocql.Session, userID string) (*ent.ShoppingCart, error) {
	var cart ent.ShoppingCart
	...
	if err := session.Query(query, userID).Scan(&cart.UserID, &cart.ItemCount, &cart.LastUpdateTimestamp); err != nil {
		if err == gocql.ErrNotFound {
			return nil, fmt.Errorf("cart for user %s not found", userID)
		}
...
func listAllCarts(session *gocql.Session) ([]*ent.ShoppingCart, error) {
	var carts []*ent.ShoppingCart
...
func main() {
	...
	newCart := &ent.ShoppingCart{ // Use pointer to ent.ShoppingCart
		UserID:              "testuser_123",
		ItemCount:           1,
